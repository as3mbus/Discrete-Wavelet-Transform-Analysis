...
def extract(img,imgcover,alpha):
    ...
    watermark =((img.astype(np.double)-imgcover.astype(np.double))/alpha).astype(np.uint8)
    return watermark

if __name__ == '__main__':
    imgOriginal=cv2.imread("/media/DATA/UDINUS/SMT 6/Advanced Image Processing/Project/Picture1.png")
    imgOriginalDWT = waveleteTransform(imgOriginal)
    height, width = imgOriginal.shape[:2]
    imgWatermark = cv2.imread("/media/DATA/UDINUS/SMT 6/Advanced Image Processing/Project/index.jpeg")
    ...
    alpha =0.007
    imgWatermarkedDWT = embed(imgOriginalDWT,imgWatermarkDWTLL,0,0,width/2,height/2,alpha)
    imgWatermarked = inverseWaveleteTransform(imgWatermarkedDWT)
    imgWatermarkedDDWT = waveleteTransform(imgWatermarked)
    watermark = extract(imgWatermarkedDDWT[0:height/2,0:width/2],imgOriginalDWT[0:height/2,0:width/2],alpha)
