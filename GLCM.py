...
def correlation(glcm, meanI, meanJ, taoI, taoJ):
    ...
    for i in range(256):
        for j in range(256):
            correlation[0] += ((i - meanI[0]) * (j - meanJ[0])
                               * glcm[i, j, 0]) / (taoI[0] * taoJ[0])
            correlation[1] += ((i - meanI[1]) * (j - meanJ[1])
                               * glcm[i, j, 1]) / (taoI[1] * taoJ[1])
            correlation[2] += ((i - meanI[2]) * (j - meanJ[2])
                               * glcm[i, j, 2]) / (taoI[2] * taoJ[2])
            # print correlation;
    return correlation
